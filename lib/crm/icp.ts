...
import { ScoringAuditAction } from "@/generated/prisma/enums"

export async function scoreAudit(orgId: string, action: ScoringAuditAction, actor: { id?: string; name?: string } | null, details?: Record<string, unknown>) {
...
export async function createICP(orgId: string, userId: string | undefined, input: IcpInput): Promise<ICPView> {
  ...
  await scoreAudit(orgId, ScoringAuditAction.ICP_CREATED, userId ? { id: userId } : null, { icpProfileId: row.id, name: row.name })
  return toView(row)
}
...
export async function updateICP(orgId: string, id: string, input: IcpInput, actor?: { id?: string; name?: string }): Promise<ICPView | null> {
  ...
  // TASK 012 §64: criteria changes invalidate existing scores; a rescore
  // job is queued by the caller when configured.
  await scoreAudit(orgId, ScoringAuditAction.ICP_UPDATED, actor ?? null, { icpProfileId: id })
  return toView(row)
}
