...
export async function createRun(orgId: string, _userId: string, input: { sourceId: string; icpId?: string; test?: boolean }) {
  const source = await prisma.leadSource.findFirst({ where: { id: input.sourceId, ...orgWhere(orgId), isActive: true }, select: { id: true } })
  if (!source) throw new Error("Source not found or inactive")
  let icp = null
  if (input.icpId) {
    icp = await prisma.iCPProfile.findFirst({ where: { id: input.icpId, ...orgWhere(orgId) }, select: { id: true, name: true } })
  ...
  const run = await prisma.scraperRun.create({
    data: {
      ...
      sourceId: source.id,
      ...(icp ? { icpId: icp.id, metadata: { icpName: icp.name, ...(input.test ? { test: true } : {}) } } : {}),
      ...(!icp && input.test ? { metadata: { test: true } } : {}),
    },
  })
...
export async function listRunEvents(orgId: string, runId: string, filters: RunEventFilters = {}) {
  ...
  const run = await prisma.scraperRun.findFirst({ where: { id: runId, ...orgWhere(orgId) }, select: { id: true } })
  if (!run) return null
...
export async function listRawPages(orgId: string, runId: string, filters: { page?: number; pageSize?: number } = {}) {
  const { page, pageSize } = parsePagination(filters)
  const where: Prisma.RawPageWhereInput = { runId, ...orgWhere(orgId) }
  const [total, data] = await Promise.all([
...
export async function getRawPage(orgId: string, id: string) {
  return prisma.rawPage.findFirst({ where: { id, ...orgWhere(orgId) } })
}
...
export async function cancelRun(orgId: string, runId: string): Promise<boolean> {
  const run = await prisma.scraperRun.findFirst({ where: { id: runId, ...orgWhere(orgId) }, select: { id: true, status: true } })
...
export async function listRawLeads(orgId: string, runId: string, filters: { page?: number; pageSize?: number } = {}) {
  const { page, pageSize } = parsePagination(filters)
  const where: Prisma.RawLeadWhereInput = { runId, ...orgWhere(orgId) }
  const [total, data] = await Promise.all([
